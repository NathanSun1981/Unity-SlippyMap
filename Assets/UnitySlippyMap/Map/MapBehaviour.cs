...
namespace UnitySlippyMap.Map
{
	...
	#region MonoBehaviour implementation
		...
		private void OnGUI ()
		{
            ...
            // *Will have to be potentially changed by TouchScript code **//
			...
			if (usesLocation
				...
				if (updatesCenterWithLocation) {
					if (UnityEngine.Input.location.lastData.longitude <= 180.0f
						...
						Debug.Log("DEBUG: Map.Update: new location: " + UnityEngine.Input.location.lastData.longitude + " " + UnityEngine.Input.location.lastData.latitude + ":  " + UnityEngine.Input.location.status);					
					} else {
